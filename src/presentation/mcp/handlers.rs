//! MCP tool and request handlers
use anyhow::Result;
use serde_json::{json, Value};
use std::time::{Duration, Instant};

use synapsis_core::core::orchestrator::AgentStatus;
use synapsis_core::domain::entities::SearchParams;
use synapsis_core::domain::crypto::PqcAlgorithm;
...
use crate::presentation::mcp::server::{ConnectionStatus, McpServer, PublishParams};

impl McpServer {
    pub(crate) fn list_tools(&self, id: &Value) -> Result<Value> {
        Ok(json!({
            "jsonrpc": "2.0",
            "id": id,
            "result": {
    ...
    pub(crate) fn call_tool(&self, id: &Value, params: &Value) -> Result<Value> {
    ...
    fn action_mem_session_start(&self, args: &Value) -> Result<Value, String> {
        ...
        if session_id.is_none() {
            ...
            match self
                .db
                .register_agent_session(agent_type, agent_instance, Some(project), None)
            {
    ...
    pub(crate) fn action_mem_context(&self, args: &Value) -> Result<Value, String> {
        let project = args
            .get("project")
            .and_then(|v| v.as_str())
            .unwrap_or("default");
    ...
    fn action_task_complete(&self, args: &Value) -> Result<Value, String> {
        ...
        let _ = self.orchestrator.complete_task(task_id, success);
    ...
    fn action_task_delegate(&self, args: &Value) -> Result<Value, String> {
        ...
        let from_agent = args
            ...
            match self.orchestrator.delegate_task(task_id, serde_json::Value::String(from_agent.into())) {
